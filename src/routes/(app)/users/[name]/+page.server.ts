...
export const load = (async ({ locals, params }) => {
  ...
  const ascentsResult = await db(async (tx) =>
    user == null
      ? null
  ...
  const firstAscents = await db(async (tx) =>
    user?.firstAscensionistFk == null && firstAscensionist == null
      ? null
      : tx.query.routesToFirstAscensionists.findMany({
          where: eq(
  ...
  const firstAscentRoutes = await db(async (tx) =>
    firstAscents == null
      ? null
